...
| 来源 | 类型 | 覆盖内容 |
|------|------|----------|
| 代码分析 | 直接 | `anthropic_protocol.go`, `chat_protocol.go`, `main.go` (~7000行) |
...
| 文件 | 行数 | 关键函数 |
|------|------|---------|
| `anthropic_protocol.go` | 120 | `convertClaudeRequest()`, `convertClaudeToolChoice()` |
| `chat_protocol.go` | 130 | `normalizeFinishReason()`, `writeProtocolValidation400()` |
| `main.go` | ~7000 | `claudeMessagesHandler()`, `claudeToOpenAIMessages()`, `claudeStreamHandler()`, `openAIToClaudeResponse()` |

...
async function loadStats(){try{const r=await fetch('/api/stats');const d=await r.json();const ms=d.models||{};const ks=Object.keys(ms);let h='<table class="tbl" id="statsTable"><thead><tr><th>模型</th><th>请求数</th><th>输入 Token</th><th>输出 Token</th><th>总计 Token</th><th>缓存读取</th><th>缓存写入</th></tr></thead><tbody>';if(!ks.length){h+='<tr><td colspan="7" class="empty-hint">暂无数据</td></tr>'}else{let tr=0,pt=0,ct=0,tt=0,cr=0,cw=0;for(const k of ks){const m=ms[k];h+='<tr><td>'+esc(k)+'</td><td>'+fmt(m.request_count)+'</td><td>'+fmt(m.prompt_tokens)+'</td><td>'+fmt(m.completion_tokens)+'</td><td>'+fmt(m.total_tokens)+'</td><td>'+fmt(m.cache_read_tokens||0)+'</td><td>'+fmt(m.cache_created_tokens||0)+'</td></tr>';tr+=m.request_count;pt+=m.prompt_tokens;ct+=m.completion_tokens;tt+=m.total_tokens;cr+=m.cache_read_tokens||0;cw+=m.cache_created_tokens||0}h+='<tr><td>总计</td><td>'+fmt(tr)+'</td><td>'+fmt(pt)+'</td><td>'+fmt(ct)+'</td><td>'+fmt(tt)+'</td><td>'+fmt(cr)+'</td><td>'+fmt(cw)+'</td></tr>'}h+='</tbody></table>';document.getElementById('statsContent').innerHTML=h}catch(e){document.getElementById('statsContent').innerHTML='<div class="empty-hint">加载失败</div>'}}
